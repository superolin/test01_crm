layui.use(['table','layer'],function(){
    ...
    /**
     * 搜索功能开发(根据"customerName"/"createMan"/"devResult"三个条件查询)
     */
    $(".search_btn").click(function(){
        ...
        tableIns.reload({
            where: { //设定异步数据接口的额外参数，任意设
                ...
                devResult: $("#devResult").val()  // 老师的代码有bug: 开发状态devResult而不是state
            }
            ,page: {
                curr: 1 // 重新从第 1 页开始
            }
    ...
    table.on("tool(saleChances)",function(obj){
        console.log(obj);
        if(obj.event==='info'){
            openCusDevPlanDialog("营销机会维护",obj.data.id);
            return ;
        }else if(obj.event == 'dev'){
            openCusDevPlanDialog("营销机会开发",obj.data.id);
            return ;
        }
    });
