layui.use(['table','layer'],function(){
    ...
    //"添加"或者"编辑"操作调用此方法, 以跳转至弹出层页面.
    //如果是"添加"无需回显效果, 如果是"编辑", 则需要回显效果, 此时url传输带回参数id,将查询数据采用model作用域,来返回填充进弹出层
    ...
    table.on("tool(roles)",function(obj){
        if(obj.event === 'edit'){               //行内的"编辑"操作
            openAddOrUpdateDialog(obj.data.id);
            return ;
        }else if(obj.event === 'del'){          //行内的"删除"操作
            layer.msg();
